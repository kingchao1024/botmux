 ...
 */
type DaemonTaskControlIntegrationInput = {
  dataDir: string;
  larkAppId: string;
...
export class DaemonTaskControlIntegration {
  readonly #input: DaemonTaskControlIntegrationInput;
  ...
  constructor(input: DaemonTaskControlIntegrationInput) {
    this.#input = input;
  ...
  issueAcceptorAuthentication(dispatchRoot: string) {
    ...
    return this.mapping(dispatchRoot)
      ? this.#input.bridge.issueAuthentication(dispatchRoot, 'acceptor')
      : undefined;
  }
  ...
  submitVerifiedReviewerVerdict(input: {
    ...
    const head = this.#input.store.appendReviewerVerdict({
      verdict: input.verdict, attestation: input.attestation, authentication,
  ...
  reviewed(dispatchRoot: string, sourceRef: string, input: {
    ...
    // A lifecycle review can originate only from submitVerifiedReviewerVerdict(), after
   ...
   */
  private terminalDelivered(input: { eventId: string; destinationId: string; receiptRef: string; deliveredAt?: string }): boolean {
   ...
   */
  private fallbackVerified(input: {
